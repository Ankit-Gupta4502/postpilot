...
export async function backfillHandler(msg: { body: unknown }) {
  ...
  while (page < MAX_PAGES) {
    ...
    if (!checkpoint || result.posts.length === 0) break
  }
  ...
  console.log(`Backfill complete for account ${socialAccountId}: ${page} page(s) synced`)
}
